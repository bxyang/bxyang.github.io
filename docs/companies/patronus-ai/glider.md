...
作者报告最终输入长度为 217–6,523 个词，平均 608 个词；这包含格式化的输入，不等于仅计算模型回答长度，更不等于 token 数。（§3.1）
...
GLIDER 使用英语训练数据，随后在多语言测试集上评测。表 4 的 BigGen 多语言 Pearson 为 0.545，高于底座的 0.290、FlowAI Judge 的 0.410 和 Llama 的 0.450，低于 GPT-4o 的 0.583 及 Claude 的 0.554。表标题写成 BigBench，但正文和公开数据入口对应 BigGen Bench 多语言部分。（表 4）
...
论文没有公开完整训练—测试去重结果或逐领域留出映射，LiveBench 的较新时间切片也不能替代全部数据的污染审计。
